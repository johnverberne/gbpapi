...
//      -mo "PCRASTER_VALUESCALE=VS_NOMINAL" LCEU_ini.tif LCEU_ini.map
  public static void geoTiff2PcRaster(File geotiffFile, File mapFile) throws IOException{
    final String[] args = { "-of", "PCRaster", "-ot", "Float32" /*"Int32"*/, "-mo", "PCRASTER_VALUESCALE=VS_NOMINAL",
        "-b", "1", geotiffFile.getAbsolutePath(), mapFile.getAbsolutePath() };
  ...
  public static void pcRaster2GeoTiff(File mapFile, File geotiffFile) throws IOException{
    final String[] args = { "-a_srs", "EPSG:28992",
        mapFile.getAbsolutePath(), geotiffFile.getAbsolutePath() };
    final ExecParameters execParams = new ExecParameters(GDAL_TRANSLATE, args);
    final Exec exec = new Exec(execParams, "");
    try {
      exec.run(new File(geotiffFile.getParent()));
